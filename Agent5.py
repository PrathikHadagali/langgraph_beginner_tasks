...
def take_action(state: AgentState) -> AgentState:
    ...
    for t in tool_calls:
        query_text = t['args']  # it's already a string
        print(f"Calling Tool: {t['name']} with query: {query_text}")

        if t['name'] not in tools_dict:
            print(f"\nTool: {t['name']} does not exist.")
            result = "Incorrect Tool name, Please Retry and Select tool from List of Available tools."
        else:
            result = tools_dict[t['name']].invoke(query_text)
            print(f"Result Length: {len(str(result))}")

        
        results.append(
            ToolMessage(
                tool_call_id=str(uuid4()),  # generate a unique ID for this tool call
                name=t['name'],
                content=str(result)
            )
        )
